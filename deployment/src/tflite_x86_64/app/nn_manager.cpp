...
NeuralNetworkManager::NeuralNetworkManager() {
    ...
    static tflite::RecordingMicroInterpreter static_interpreter(model, *resolver, tensor_arena, kTensorArenaSize, error_reporter);                   
    ...
    static_interpreter.GetMicroAllocator().PrintAllocations();
...
bool logResults(uint32_t feature_number, TfLiteTensor* output, uint32_t duration_ms) {
    ...
    printf("No, %d, ArgMax, %ld, Value, %d, Duration, %d, x86\n", feature_number, max_i, max_v, duration_ms);    
    fprintf(log, "No, %d, ArgMax, %ld, Value, %d, Duration, %d, x86\n", feature_number, max_i, max_v, duration_ms);    
    fflush(log);
    fclose(log);
...
void NeuralNetworkManager::run_inference() {
    ...
    switch (status)
    {
        case kTfLiteOk:
        {
            ...
            logResults(this->input_number, output, inference_time);
