...
def main():
  ...
  models_base_path = 'ext/ACDNet3/tf/resources/pruned_trained_models/taylor'
  
  # models_path = os.path.join(cwd_path, models_base_path)
  # model_files = [f for f in os.listdir(models_path) if f.endswith(('h5','hdf5'))]
  
  model_files = [
    '20khz_SP41_taylor_l0_full_80_85.25.h5',
    '20khz_SP41_taylor_full_85_84.75.h5']

  dtypes = ['int8']

  #data_sets = data_support.keys()
  data_sets = ['aug-data-20khz']

  crop_size = 10
  results = []
  report_datetime = datetime.datetime.now()
  results_path = f'results/results_{report_datetime:%y%m%d_%H%M}.npy'

  for model_file in model_files:
    for dtype in dtypes:

      converter = KerasConverter(os.path.join(models_base_path, model_file), dtype, cwd_path)
      converter.load_model()
      input_size, output_size = converter.get_model_size()

      for set_name in data_sets:
        data_set = data_support[set_name]
        
        if data_set['input_size'] != input_size:
            print(f'Skipping {set_name} for input size {data_set["input_size"]}!={input_size}')
            continue

        try:
          # Will attempt to download data, if it does not yet exist locally 
          download(set_name)

          # Setup the data paths for the TF and TFLite accuracy tests and TFLite conversion
          test_dataset_path = get_data_path(set_name, 'test')
          representative_dataset_path = get_data_path(set_name, 'representative')
          if representative_dataset_path is None:
              representative_dataset_path = test_dataset_path

          converter.set_data_path(test_dataset_path, representative_dataset_path)
